  ...
  new Vuex.Store({
    ...
    mutations: {
      ...
      offlineFallback(state) {
        ...
        nearest.lines = [];
        const sameStations = scored.filter(
          station => nearest.station_g_cd === station.station_g_cd
        );
        sameStations.forEach(station => {
          const matchedLine = state.dic.lines.filter(
            line => line.line_cd === station.line_cd
          );
